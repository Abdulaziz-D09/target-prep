...
const mdPath = '/Users/abdulazizdavronov/Downloads/MQB(1-924).md';
...
for (let i = 1; i < chunks.length; i += 2) {
  ...
  if (question && finalAnswer !== undefined && finalAnswer !== "") {
    questions.push({
      id: qId,
      type: options.length > 0 ? "Math" : "Math (SPR)",
    ...
    unparsed++;
    // console.log("Failed to parse completely:", qId);
  }
}

console.log(`Successfully parsed ${questions.length} questions. Failed: ${unparsed}`);
fs.writeFileSync('src/data/math_bank.json', JSON.stringify(questions, null, 2));
