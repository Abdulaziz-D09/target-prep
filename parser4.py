...
def parse_module(text, start_marker, end_marker=None):
    ...
    for i in range(len(parts) - 1):
        ...
        # Make the regex more robust to whitespace. Often options are "A. ", "B. ", "C. ", "D. "
        # It could be "\nA. ", " A. ", or even multiple newlines.
        opt_matches = list(re.finditer(r'(?:^|\s)([A-D])\.\s+', q_chunk))
        
        # Sometimes there are stray matches in the text, so we only want the sequence A, B, C, D at the very end
        # We can find the LAST four matches that form A, B, C, D
        abcd_matches = []
...
# Print debug info to see how many options we got for each
for q in eng_m2:
    if len(q['options']) == 0:
        print(f"Warning: English M2 Q{q['num']} has 0 options!")
for q in math_m2:
    if len(q['options']) == 0:
        pass # Expected for Math SPR
...
print(f"Re-injected properly with flexible option parsing! {len(eng_m2)} eng, {len(math_m2)} math.")
